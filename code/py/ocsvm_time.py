...
file_path = '/Users/hk00569ml/代码/fake deposit/ocsvm_test_results.pkl'
...
# --- 7. Plot Bottom: Histogram (Fixed Y-axis) ---

weights = np.ones_like(plot_data) / len(plot_data)

ax_hist.hist(plot_data, bins=log_bins, density=False, weights=weights,
...
# Optional: Format Y-axis as percentage (e.g., 0.1 -> 10%)
# ax_hist.yaxis.set_major_formatter(FuncFormatter(lambda y, _: f'{y:.0%}')) 

# --- 10. Labels and Alignment Settings ---
ax_hist.set_xlabel('OC-SVM detection time (ms)', fontsize=hzysfontsize, labelpad=10)
...
save_path = os.path.join(output_dir, 'ocsvm_dist.pdf')

# Save image (no bbox_inches='tight' to ensure alignment)
